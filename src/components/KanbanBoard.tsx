...
export default function KanbanBoard() {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const draggedCard = sourceColumn.find(card => card.id === draggableId);

    if (!draggedCard) return;

    const newSourceColumn = sourceColumn.filter(card => card.id !== draggableId);
    const newDestColumn = [...destColumn];
    newDestColumn.splice(destination.index, 0, draggedCard);
